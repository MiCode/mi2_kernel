...
static int __init parse_tag_powerup_reason(const struct tag *tag)
{
	...
	printk(KERN_INFO "%s: powerup reason=0x%x\n", __func__, get_powerup_reason());
	return 0;
}

__tagtable(ATAG_POWERUP_REASON, parse_tag_powerup_reason);

static int __init parse_tag_hwversion(const struct tag *tag)
{
	set_hw_version(tag->u.hwversion.hwversion);
	printk(KERN_INFO "%s: hwversion=0x%x\n", __func__, get_hw_version());
	return 0;
}

__tagtable(ATAG_HW_VERSION, parse_tag_hwversion);
